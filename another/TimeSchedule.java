...
//单机进程调度，不同进程间不互容，动态规划？图在Visio-调度 ，难点在保存两个index值及递归出口
public class TimeSchedule {
	public static int tSchedule(int[] endTime, int[] dur, int[] values, int index, int ltindex,int price){
		...
		if(index == -1){
			...
			int A = tSchedule(endTime, dur, values, index1, index, price+values[index]);//选择下标为index的任务
			System.out.println("\t B~~~~~~~~~~");
			int B = tSchedule(endTime, dur, values, index-1, ltindex, price);//不选下标为index的任务
			int pr = (A >= B ? A : B);
